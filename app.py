...
def generate_mega_word_bank():
    # 填寫說明：
    # 請把 2000 個單字照順序填入下方的大清單中。
    # 格式必須是： {"en": "英文單字", "zh": "中文翻譯"},
    all_2000_words = [
        # ----------------------------------------------------
        # 【第 1 組】 (第 1 ~ 50 個單字)
        # ----------------------------------------------------
        ...
        {"en": "computer", "zh": "電腦"},
        {"en": "doctor", "zh": "醫生"},
        # ...請在這裡依序填入第 53 到第 100 個單字...

        # ----------------------------------------------------
        # 【第 3 組】 到 【第 39 組】 請依此類推按順序往下填寫
        # ----------------------------------------------------

        # ----------------------------------------------------
        # 【第 40 組】 (第 1951 ~ 2000 個單字)
        # ----------------------------------------------------
        {"en": "zebra", "zh": "斑馬"},
        {"en": "zoo", "zh": "動物園"} # 注意：最後一個單字結尾不需要逗號
    ]
    
    # 🤖 後台自動分類核心：每 50 個單字切成一組
    mega_bank = {}
    for i in range(1, 41):
        start_index = (i - 1) * 50
        end_index = i * 50
        # 切片取出對應範圍的 50 個單字，綁定到組別 i
        mega_bank[i] = all_2000_words[start_index:end_index]
